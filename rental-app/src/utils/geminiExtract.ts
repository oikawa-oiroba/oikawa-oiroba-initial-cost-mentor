...
export const extractPropertyDataFromImage = async (
  ...
  const prompt = [
    ...
    "- roomNumber: 部屋番号",
    '- agencyFeeType: 仲介手数料の種類。"1.1"=1ヶ月+消費税、"0.55"=0.5ヶ月+消費税、"118000"=118000円固定、"0"=無料・AD物件、"custom"=その他固定金額',
    ...
    "- 見つからない項目はnull",
    "",
    "JSONのみで返してください。マークダウン記法は使わないでください。"
  ].join("\n");
  ...
  if (!jsonMatch) {
    ...
    throw new Error("解析に失敗しました。もう一度お試しください。");
  }
