...
export const RentalCalculator = () => {
  ...
  const analyzeImage = async (file: File) => {
    ...
    try {
      ...
      if (err.message?.includes("403") || err.message?.includes("API key")) {
        setError("APIキーが無効です。正しいGemini APIキーを設定してください。");
        setShowApiKeyInput(true);
      } else if (err.message?.includes("429")) {
        setError("APIの利用制限に達しました。しばらくしてから再試行してください。");
      } else {
        setError(`解析エラー: ${err.message || "不明なエラー"}`);
        setShowApiKeyInput(true);
      }
    } finally {
  ...
  const calculate = () => {
    if (!form.rent || !form.moveInDate) { setError("家賃と入居予定日を入力してください。"); return; }
    setError("");
    const otherFees = [];
    ...
    const result = calculateCostBreakdown(
      parseFloat(form.rent), parseFloat(form.managementFee || "0"), form.moveInDate,
      parseFloat(form.sanitationFee || "0"), parseFloat(form.supportFee || "0"), parseFloat(form.contractFee || "0"),
      form.depositMonths === "custom" ? parseFloat(form.customDeposit || "0") / parseFloat(form.rent) : parseFloat(form.depositMonths),
      form.keyMoneyMonths === "custom" ? parseFloat(form.customKeyMoney || "0") / parseFloat(form.rent) : parseFloat(form.keyMoneyMonths),
      form.agencyFeeType, form.customAgencyFee ? parseFloat(form.customAgencyFee) : undefined,
      form.guaranteeFeeRate === "custom" ? parseFloat(form.customGuaranteeFeeRate || "0") / 100 : parseFloat(form.guaranteeFeeRate),
      form.hasRentFree, form.hasRentFree ? parseFloat(form.rentFreeMonths) : 0,
      { hasInsuranceFee: form.hasInsuranceFee, hasKeyExchangeFee: form.hasKeyExchangeFee, hasCleaningFee: form.hasCleaningFee,
        insuranceFeeAmount: form.insuranceFeeAmount, keyExchangeFeeAmount: form.keyExchangeFeeAmount, cleaningFeeAmount: form.cleaningFeeAmount, otherFees }
    );
    setBreakdown(result);
  };
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-4">
            ...
            <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
              ...
              <div className="flex gap-2">
                <input type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)}
                  placeholder="AIzaSy..." className="flex-1 border border-amber-200 rounded px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-amber-400" />
                <button onClick={saveApiKey} className="px-3 py-1.5 bg-amber-500 text-white rounded text-xs font-medium hover:bg-amber-600">保存</button>
              </div>
              <p className="text-xs text-amber-600 mt-1">
                <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer" className="underline">Google AI Studioで取得</a>
              </p>
            </div>
          )}
          ...
          <div className="mt-2 flex items-center gap-3">
            {!showApiKeyInput && (
              <button onClick={() => setShowApiKeyInput(true)} className="text-xs text-purple-600 underline hover:text-purple-800">
                {apiKey ? "APIキーを変更する" : "APIキーを設定する"}
              </button>
            )}
            {apiKey && !showApiKeyInput && <span className="text-xs text-gray-400">キー設定済み ✓</span>}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <div className={sectionClass}>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div><label className={labelClass}>物件名</label>
                <input className={inputClass} placeholder="〇〇マンション" value={form.propertyName} onChange={e => form.setPropertyName(e.target.value)} /></div>
              <div><label className={labelClass}>部屋番号</label>
                <input className={inputClass} placeholder="101" value={form.roomNumber} onChange={e => form.setRoomNumber(e.target.value)} /></div>
            </div>
            <div><label className={labelClass}>物件URL（任意）</label>
              <input className={inputClass} placeholder="https://suumo.jp/..." value={form.propertyUrl} onChange={e => form.setPropertyUrl(e.target.value)} /></div>
          </div>

          <div className={sectionClass}>
            <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">基本情報</h3>
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div><label className={labelClass}>家賃（円）<span className="text-red-500">*</span></label>
                <input className={inputClass} type="number" placeholder="80000" value={form.rent} onChange={e => form.setRent(e.target.value)} /></div>
              <div><label className={labelClass}>管理費・共益費（円）</label>
                <input className={inputClass} type="number" placeholder="5000" value={form.managementFee} onChange={e => form.setManagementFee(e.target.value)} /></div>
            </div>
            <div><label className={labelClass}>入居予定日<span className="text-red-500">*</span></label>
              <input className={inputClass} type="date" value={form.moveInDate} onChange={e => form.setMoveInDate(e.target.value)} /></div>
          </div>

          <div className={sectionClass}>
            <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">契約条件</h3>
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div><label className={labelClass}>敷金</label>
                <select className={inputClass} value={form.depositMonths} onChange={e => form.setDepositMonths(e.target.value)}>
                  {DEPOSIT_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select>
                {form.depositMonths === "custom" && <input className={`${inputClass} mt-1`} type="number" placeholder="金額（円）" value={form.customDeposit} onChange={e => form.setCustomDeposit(e.target.value)} />}</div>
              <div><label className={labelClass}>礼金</label>
                <select className={inputClass} value={form.keyMoneyMonths} onChange={e => form.setKeyMoneyMonths(e.target.value)}>
                  {KEY_MONEY_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select>
                {form.keyMoneyMonths === "custom" && <input className={`${inputClass} mt-1`} type="number" placeholder="金額（円）" value={form.customKeyMoney} onChange={e => form.setCustomKeyMoney(e.target.value)} />}</div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className={labelClass}>仲介手数料</label>
                <select className={inputClass} value={form.agencyFeeType} onChange={e => form.setAgencyFeeType(e.target.value)}>
                  {AGENCY_FEE_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select>
                {form.agencyFeeType === "custom" && <input className={`${inputClass} mt-1`} type="number" placeholder="金額（税抜・円）" value={form.customAgencyFee} onChange={e => form.setCustomAgencyFee(e.target.value)} />}</div>
              <div><label className={labelClass}>保証会社料率</label>
                <select className={inputClass} value={form.guaranteeFeeRate} onChange={e => form.setGuaranteeFeeRate(e.target.value)}>
                  {GUARANTEE_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select>
                {form.guaranteeFeeRate === "custom" && <input className={`${inputClass} mt-1`} type="number" placeholder="料率（%）" value={form.customGuaranteeFeeRate} onChange={e => form.setCustomGuaranteeFeeRate(e.target.value)} />}</div>
            </div>
          </div>
          ...
          <div className={sectionClass}>
            ...
            {form.hasRentFree && (
              <div className="mt-2"><label className={labelClass}>フリーレント期間</label>
                <select className={inputClass} value={form.rentFreeMonths} onChange={e => form.setRentFreeMonths(e.target.value)}>
                  {["1","2","3"].map(m => <option key={m} value={m}>{m}ヶ月</option>)}</select></div>
            )}
          </div>
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={label} className="flex items-center gap-3">
                  ...
                  <input type="number" value={amount} onChange={e => setAmount(e.target.value)} disabled={!check}
              ...
              {[
                ...
                <div key={i} className="flex items-center gap-3">
                  ...
                  <input type="text" value={name} onChange={e => setName(e.target.value)} disabled={!has} placeholder="費用名"
                    className={`w-32 border border-gray-200 rounded-lg px-2 py-1.5 text-sm ${!has ? "bg-gray-100 text-gray-400" : "bg-white"}`} />
                  <input type="number" value={amount} onChange={e => setAmount(e.target.value)} disabled={!has}
          ...
          <button onClick={calculate} className="w-full bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white font-semibold py-3 rounded-xl transition-colors text-sm">
            初期費用を計算する
          </button>
        </div>
        ...
        {breakdown && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mt-4">
            ...
            {form.propertyUrl && (
              <a href={form.propertyUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-500 underline break-all">{form.propertyUrl}</a>
            )}
            ...
            {[
              ...
              const items = Object.values(breakdown.costs as Record<string, any>).filter((c: any) => c.category === key && c.amount !== 0);
              const subtotal = breakdown.subtotals[key];
              if (items.length === 0) return null;
              const colorMap: Record<string, string> = {
                blue: "bg-blue-50 border-blue-100", purple: "bg-purple-50 border-purple-100", green: "bg-green-50 border-green-100",
              };
              const headerColor: Record<string, string> = {
                blue: "text-blue-800 bg-blue-100", purple: "text-purple-800 bg-purple-100", green: "text-green-800 bg-green-100",
              };
              return (
            ...
            <p className="text-xs text-gray-400 text-center mt-4">※この計算結果はあくまで概算です。実際の金額は契約内容により異なります。</p>
          </div>
        )}
      </div>
    </div>
